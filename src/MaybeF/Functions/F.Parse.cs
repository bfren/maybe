...
public static partial class F
{
	...
	internal static Maybe<T> Parse<T>(ReadOnlySpan<char> value, TryParseSpan<T> tryParse) =>
		tryParse(value, out var result) switch
		{
			...
			false =>
				None<T>(new R.UnableToParseValueAs(typeof(T), value.ToString()))
		};
	...
	internal static Maybe<T> Parse<T>(string value, TryParseString<T> tryParse) =>
		tryParse(value, out var result) switch
		{
			...
			false =>
				None<T>(new R.UnableToParseValueAs(typeof(T), value))
		};
	...
	public static partial class R
	{
		...
		/// <param name="Value">Input value</param>
		public sealed record class UnableToParseValueAs(Type Type, string Value) : IReason;
	}
}
