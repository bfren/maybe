...
public class SomeIfAsync_Tests : Tests.Maybe.Abstracts.SomeIfAsync_Tests
{
	...
	{
		await Test01((predicate, value, handler) => value.SomeIfAsync(predicate, handler)).ConfigureAwait(false);
		await Test01((predicate, value, handler) => value.SomeIfAsync(_ => predicate(), handler)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test02_Exception_Thrown_By_Value_Func_Calls_Handler_Returns_None()
	{
		await Test02((predicate, value, handler) => value.SomeIfAsync(predicate, handler)).ConfigureAwait(false);
		await Test02((predicate, value, handler) => value.SomeIfAsync(_ => predicate(), handler)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test04_Predicate_True_With_Value_Func_Returns_Some()
	{
		await Test04((predicate, value, handler) => value.SomeIfAsync(predicate, handler)).ConfigureAwait(false);
		await Test04((predicate, value, handler) => value.SomeIfAsync(_ => predicate(), handler)).ConfigureAwait(false);
	}
	...
	{
		await Test07((predicate, value, handler) => value.SomeIfAsync(predicate, handler)).ConfigureAwait(false);
	}
