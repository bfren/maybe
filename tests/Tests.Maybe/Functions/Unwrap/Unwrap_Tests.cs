...
public class Unwrap_Tests : Tests.Maybe.Abstracts.Unwrap_Tests
{
	...
	{
		Test00((mbe, ifNone) => MaybeF.Unwrap(mbe, ifNone));
	}

	[Fact]
	public override void Test01_None_With_Reason_Runs_IfNone_Func_Passes_Reason_Returns_Value()
	{
		Test01((mbe, ifNone) => MaybeF.Unwrap(mbe, ifNone));
	}

	[Fact]
	public override void Test02_Some_Returns_Value()
	{
		Test02(mbe => MaybeF.Unwrap(mbe, Substitute.For<Func<int>>()));
		Test02(mbe => MaybeF.Unwrap(mbe, Substitute.For<Func<IReason, int>>()));
	}
}
