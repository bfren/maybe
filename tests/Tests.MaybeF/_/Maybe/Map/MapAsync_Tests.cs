...
public class MapAsync_Tests : Abstracts.MapAsync_Tests
{
	...
	{
		await Test00((mbe, map, handler) => mbe.MapAsync(map, handler)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test01_Exception_Thrown_Without_Handler_Returns_None_With_UnhandledExceptionReason()
	{
		await Test01((mbe, map, handler) => mbe.MapAsync(map, handler)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test02_Exception_Thrown_With_Handler_Calls_Handler_Returns_None()
	{
		await Test02((mbe, map, handler) => mbe.MapAsync(map, handler)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test03_If_None_Returns_None()
	{
		await Test03((mbe, map, handler) => mbe.MapAsync(map, handler)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test04_If_None_With_Reason_Returns_None_With_Same_Reason()
	{
		await Test04((mbe, map, handler) => mbe.MapAsync(map, handler)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test05_If_Some_Runs_Map_Function()
	{
		await Test05((mbe, map, handler) => mbe.MapAsync(map, handler)).ConfigureAwait(false);
	}
}
