...
public class SwitchAsync_Tests : Abstracts.SwitchAsync_Tests
{
	...
	{
		await Test06((mbe, some) => mbe.SwitchAsync(some, F.Some(Rnd.Str).AsTask()));
	...
	{
		await Test08((mbe, some) => mbe.SwitchAsync(some, F.Some(Rnd.Str).AsTask()));
	...
	public override async Task Test10_If_Unknown_Maybe_Returns_UnknownMaybeTypeMsg()
	{
		...
		await Test10(mbe => mbe.SwitchAsync(some, F.Some(Rnd.Str).AsTask()));
		await Test10(mbe => mbe.SwitchAsync(some, Substitute.For<Func<Task<Maybe<string>>>>()));
	}
