...
public class IfSomeAsync_Tests : Abstracts.IfSomeAsync_Tests
{
	...
	{
		await Test01((mbe, ifSome) => mbe.AsTask.IfSomeAsync(x => ifSome(x))).ConfigureAwait(false);
	...
	{
		await Test02((mbe, ifSome) => mbe.AsTask.IfSomeAsync(x => ifSome(x))).ConfigureAwait(false);
