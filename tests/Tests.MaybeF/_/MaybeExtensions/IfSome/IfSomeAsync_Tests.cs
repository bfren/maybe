...
public class IfSomeAsync_Tests : Abstracts.IfSomeAsync_Tests
{
	...
	{
		await Test00((mbe, ifSome) => mbe.AsTask.IfSomeAsync(x => _ = ifSome(x))).ConfigureAwait(false);
	...
	{
		await Test01((mbe, ifSome) => mbe.AsTask.IfSomeAsync(x => _ = ifSome(x))).ConfigureAwait(false);
		await Test01((mbe, ifSome) => mbe.AsTask.IfSomeAsync(ifSome)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test02_Some_Runs_IfSome_Func_And_Returns_Original_Option()
	{
		await Test02((mbe, ifSome) => mbe.AsTask.IfSomeAsync(x => _ = ifSome(x))).ConfigureAwait(false);
		await Test02((mbe, ifSome) => mbe.AsTask.IfSomeAsync(ifSome)).ConfigureAwait(false);
	}
}
