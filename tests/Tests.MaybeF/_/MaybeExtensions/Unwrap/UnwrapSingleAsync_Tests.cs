...
public class UnwrapSingleAsync_Tests : Abstracts.UnwrapSingleAsync_Tests
{
	...
	{
		await Test01(mbe => mbe.UnwrapAsync(x => x.SingleValue<int>(null, null, null))).ConfigureAwait(false);
	}
	...
	{
		await Test04((mbe, noItems) => mbe.UnwrapAsync(x => x.SingleValue<int>(noItems, null, null))).ConfigureAwait(false);
	}
	...
	{
		await Test06((mbe, tooMany) => mbe.UnwrapAsync(x => x.SingleValue<int>(null, tooMany, null))).ConfigureAwait(false);
	}
	...
	{
		await Test08((mbe, notAList) => mbe.UnwrapAsync(x => x.SingleValue<int>(null, null, notAList))).ConfigureAwait(false);
	}
	...
	{
		await Test10(mbe => mbe.UnwrapAsync(x => x.SingleValue<int>(null, null, null))).ConfigureAwait(false);
	}
}
