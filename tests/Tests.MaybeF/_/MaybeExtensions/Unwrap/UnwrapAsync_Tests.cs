...
public class UnwrapAsync_Tests : Abstracts.UnwrapAsync_Tests
{
	...
	{
		await Test00((mbe, ifNone) => mbe.UnwrapAsync(x => x.Value(ifNone())));
		await Test00((mbe, ifNone) => mbe.UnwrapAsync(x => x.Value(ifNone)));
	}
	...
	{
		await Test01((mbe, ifNone) => mbe.UnwrapAsync(x => x.Value(ifNone)));
	}

	[Fact]
	public override async Task Test02_Some_Returns_Value()
	{
		await Test02(mbe => mbe.UnwrapAsync(x => x.Value(Rnd.Int)));
		await Test02(mbe => mbe.UnwrapAsync(x => x.Value(Substitute.For<Func<int>>())));
		await Test02(mbe => mbe.UnwrapAsync(x => x.Value(Substitute.For<Func<IMsg, int>>())));
	}
}
