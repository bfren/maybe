...
public class SwitchIfAsync_Tests : Abstracts.SwitchIfAsync_Tests
{
	...
	public override async Task Test02_None_Returns_Original_None()
	{
		...
		await Test02((mbe, check) => mbe.SwitchIfAsync(check, ifFalse));
	}
	...
	{
		await Test06((mbe, check, ifTrue) => mbe.SwitchIfAsync(check, ifTrue, null));
	}
	...
	{
		await Test07((mbe, check, ifFalse) => mbe.SwitchIfAsync(check, null, ifFalse));
		await Test07((mbe, check, ifFalse) => mbe.SwitchIfAsync(check, x => ifFalse(x).Reason));
	}

	[Fact]
	public override async Task Test08_Check_Returns_True_Runs_IfTrue_Returns_Value()
	{
		await Test08((mbe, check, ifTrue) => mbe.SwitchIfAsync(check, ifTrue, null));
	}

	[Fact]
	public override async Task Test09_Check_Returns_False_Runs_IfFalse_Returns_Value()
	{
		await Test09((mbe, check, ifFalse) => mbe.SwitchIfAsync(check, null, ifFalse));
		await Test09((mbe, check, ifFalse) => mbe.SwitchIfAsync(check, x => ifFalse(x).Reason));
	}
