...
public class MapAsync_Tests : Abstracts.MapAsync_Tests
{
	...
	{
		await Test02((mbe, map, handler) => mbe.AsTask.MapAsync(x => map(x).GetAwaiter().GetResult(), handler)).ConfigureAwait(false);
		await Test02((mbe, map, handler) => mbe.AsTask.MapAsync(map, handler)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test03_If_None_Returns_None()
	{
		await Test03((mbe, map, handler) => mbe.AsTask.MapAsync(x => map(x).GetAwaiter().GetResult(), handler)).ConfigureAwait(false);
		await Test03((mbe, map, handler) => mbe.AsTask.MapAsync(map, handler)).ConfigureAwait(false);
	}
	...
	{
		await Test05((mbe, map, handler) => mbe.AsTask.MapAsync(x => map(x).GetAwaiter().GetResult(), handler)).ConfigureAwait(false);
		await Test05((mbe, map, handler) => mbe.AsTask.MapAsync(map, handler)).ConfigureAwait(false);
	}
}
