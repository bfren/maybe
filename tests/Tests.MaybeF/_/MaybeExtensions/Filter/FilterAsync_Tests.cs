...
public class FilterAsync_Tests : Abstracts.FilterAsync_Tests
{
	...
	{
		await Test01((mbe, predicate) => mbe.AsTask().FilterAsync(x => H.GetResult(predicate(x))));
	...
	{
		await Test02((mbe, predicate) => mbe.AsTask().FilterAsync(x => H.GetResult(predicate(x))));
	...
	{
		await Test03((mbe, predicate) => mbe.AsTask().FilterAsync(x => H.GetResult(predicate(x))));
	...
	{
		await Test04((mbe, predicate) => mbe.AsTask().FilterAsync(x => H.GetResult(predicate(x))));
