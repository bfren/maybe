...
public class BindAsync_Tests : Abstracts.BindAsync_Tests
{
	...
	{
		await Test00(
			(mbe, bind) => mbe.AsTask().BindAsync(x => H.GetResult(bind(x))),
			(mbe, bind) => mbe.AsValueTask().BindAsync(x => H.GetResult(bind(x)))
		);
		await Test00(
			(mbe, bind) => mbe.AsTask().BindAsync(bind),
			(mbe, bind) => mbe.AsValueTask().BindAsync(bind)
		);
	}
