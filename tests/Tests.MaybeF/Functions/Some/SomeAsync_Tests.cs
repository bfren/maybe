...
public class SomeAsync_Tests : Abstracts.SomeAsync_Tests
{
	...
	{
		await Test02((val, handler) => F.SomeAsync(val, handler)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test03_Nullable_Exception_Thrown_With_Handler_Returns_None_Calls_Handler()
	{
		await Test03((val, nullable, handler) => F.SomeAsync(val, nullable, handler)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test04_Null_Input_Returns_None()
	{
		await Test04((val, handler) => F.SomeAsync(val, handler)).ConfigureAwait(false);
	}
	...
	{
		await Test06((val, nullable, handler) => F.SomeAsync(val, nullable, handler)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test07_Not_Null_Returns_Some()
	{
		await Test07((val, handler) => F.SomeAsync(val, handler)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test08_Nullable_Not_Null_Returns_Some()
	{
		await Test08((val, nullable, handler) => F.SomeAsync(val, nullable, handler)).ConfigureAwait(false);
	}
}
