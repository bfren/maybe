...
public class FilterAsync_Tests : Abstracts.FilterAsync_Tests
{
	...
	public override async Task Test00_If_Unknown_Maybe_Returns_None_With_UnhandledExceptionMsg()
	{
		...
		await Test00(
			mbe => F.FilterAsync(mbe, taskPredicate),
			mbe => F.FilterAsync(mbe, valueTaskPredicate)
		);
		await Test00(
			mbe => F.FilterAsync(mbe.AsTask(), taskPredicate),
			mbe => F.FilterAsync(mbe.AsValueTask(), valueTaskPredicate)
		);
	}
	...
	{
		await Test01(
			(mbe, predicate) => F.FilterAsync(mbe, predicate),
			(mbe, predicate) => F.FilterAsync(mbe, predicate)
		);
		await Test01(
			(mbe, predicate) => F.FilterAsync(mbe.AsTask(), predicate),
			(mbe, predicate) => F.FilterAsync(mbe.AsValueTask(), predicate)
		);
	}
